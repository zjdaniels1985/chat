    ...
    this.listener = window.addEventListener('message', (event) => {
      ...
      document.addEventListener('keyup', function (e) {
        if (e.keyCode === 9 ) {
          ...
          if (this.chatType == "REPORT"){
            this.chatType = "OTHER"
            $(".report").attr("fill-opacity", "0.25")
            $(".ooc").attr("fill-opacity", "0.25")
            $(".other").attr("fill-opacity", "1.0")
            post('http://chat/changeChatType', JSON.stringify({ chat: "other"}));
          }
          else if(this.chatType == "OTHER") {
            this.chatType = "OOC"
            $(".report").attr("fill-opacity", "0.25")
            $(".ooc").attr("fill-opacity", "1.0")
            $(".other").attr("fill-opacity", "0.25")
            post('http://chat/changeChatType', JSON.stringify({ chat: "ooc"}));
          }
          else {
            this.chatType = "REPORT"
            $(".report").attr("fill-opacity", "1.0")
            $(".ooc").attr("fill-opacity", "0.25")
            post('http://chat/changeChatType', JSON.stringify({ chat: "report"}));
          }
        }
      }, false);
  },
